...
1. Get the values file of the Snipe-IT chart or an existing installation (release).
    ...
    ```sh
    helm show values moekai/snipe-it > values.yaml
    ```
...
3. Install a new release for Snipe-IT or upgrade an existing Snipe-IT release:
    ...
    ```sh
    helm upgrade --install ${releaseName} moekai/snipe-it --namespace ${namespace} --create-namespace --values values.yaml --wait
    ```
