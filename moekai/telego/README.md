...
Deploy [`moekai/postgres-agent`](../postgres-agent/) with `postgres.mode.create` set to `true`. This step can be skipped if you have an existing PostgreSQL database.
...
```sh
helm install $release_name moekai/telego --namespace $namespace --create-namespace --values values.yaml --wait
```
...
```sh
helm upgrade --install $release_name moekai/telego \
--namespace $namespace \
--values values.yaml \
...
Deploy [`moekai/postgres-agent`](../postgres-agent/) with `postgres.mode.drop` set to `true`.
