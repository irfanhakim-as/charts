...
1. Get the values file of the Syncthing chart or an existing installation (release).
    ...
    ```sh
    helm show values moekai/syncthing > values.yaml
    ```
...
3. Install a new release for Syncthing or upgrade an existing Syncthing release:
    ...
    ```sh
    helm upgrade --install ${releaseName} moekai/syncthing --namespace ${namespace} --create-namespace --values values.yaml --wait
    ```
